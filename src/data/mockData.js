...
/** Helper functions for category filtering and lookups */

export function filterProductsByCategory(productList, categoryId) {
  if (categoryId === "All") return productList;
  return productList.filter((p) => p.categoryId === categoryId);
}

export function getCategoryLabel(categoryIdOrObject) {
  // Handle both ID and object formats
  if (typeof categoryIdOrObject === "object") {
    return categoryIdOrObject.name || "Unknown";
  }
  const c = categories.find((x) => x.categoryId === categoryIdOrObject);
  return c?.name ?? `Category ${categoryIdOrObject}`;
}
